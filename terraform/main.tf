terraform {
  ...
  backend "s3" {
    ...
    key                         = "deployments/curriculum-planner-for-enrollment/terraform.tfstate"
    region                      = "us-east-1"
...
resource "digitalocean_project" "main" {
  name        = "ab-curriculum-planner-for-enrollment"
  description = "AgentBridge: Curriculum Planner for Enrollment"
...
resource "digitalocean_vpc" "main" {
  name        = "ab-curriculum-planner-for-enrollment-vpc"
  region      = var.region
  ip_range    = var.vpc_cidr
  description = "Isolated VPC for AgentBridge Curriculum Planner for Enrollment"
}
...
resource "digitalocean_ssh_key" "deploy" {
  ...
  name       = "ab-curriculum-planner-for-enrollment-deploy"
...
resource "digitalocean_firewall" "main" {
  name = "ab-curriculum-planner-for-enrollment-fw"
...
resource "digitalocean_droplet" "main" {
  ...
  tags = concat([
    "agentbridge",
    "blueprint-curriculum-planner-for-enrollment",
    var.environment,
...
resource "digitalocean_volume" "data" {
  ...
  region                  = var.region
  name                    = "ab-curriculum-planner-for-enrollment-data"
  size                    = var.additional_storage_gb
  initial_filesystem_type = "ext4"
  description             = "Persistent data for AgentBridge Curriculum Planner for Enrollment"

  tags = ["agentbridge", "blueprint-curriculum-planner-for-enrollment", "data"]
}
...
resource "digitalocean_spaces_bucket" "artifacts" {
  ...
  name   = "ab-curriculum-planner-for-enrollment-artifacts"
...
resource "digitalocean_loadbalancer" "main" {
  ...
  name   = "ab-curriculum-planner-for-enrollment-lb"
  region = var.region
  vpc_uuid = digitalocean_vpc.main.id
...
resource "digitalocean_monitor_alert" "cpu_high" {
  ...
  description = "AgentBridge curriculum-planner-for-enrollment: CPU > 85% for 5 min"
}
...
resource "digitalocean_monitor_alert" "memory_high" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.main.id]
  description = "AgentBridge curriculum-planner-for-enrollment: Memory > 90% for 5 min"
}
...
resource "digitalocean_monitor_alert" "disk_high" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.main.id]
  description = "AgentBridge curriculum-planner-for-enrollment: Disk > 85%"
}
